...
import type { IWorkRepository, CreateWorkInput, UpdateWorkInput } from './IWorkRepository';
import type {
	WorkItemEntity,
	WorkImageEntity,
...
export class WorkRepository implements IWorkRepository {
	async getListing(page = 1, pageSize = 12, onlyVisible = true): Promise<WorkListingResult> {
	...
	async getById(id: string): Promise<WorkItemWithImagesEntity | null> {
		const itemRes = await db.select().from(workItems).where(eq(workItems.id, id)).limit(1);
		if (itemRes.length === 0) return null;
		const item = itemRes[0];
	...
	async createWork(input: CreateWorkInput): Promise<WorkItemEntity> {
		...
		const maxOrder = allItems.length > 0 ? Math.max(...allItems.map((w) => w.displayOrder)) : -1;

		let slug = generateSlug(input.title);
		const existingSlug = await db.select().from(workItems).where(eq(workItems.slug, slug)).limit(1);
	...
	async togglePin(id: string, isPinned: boolean): Promise<void> {
		if (isPinned) {
			...
			const maxPinnedOrder =
				pinned.length > 0 ? Math.max(...pinned.map((p) => p.pinnedOrder ?? 0)) : -1;
			await db
				.update(workItems)
				.set({ isPinned: true, pinnedOrder: maxPinnedOrder + 1, updatedAt: new Date() })
	...
	async reorderPinned(id: string, direction: 'up' | 'down'): Promise<void> {
		...
		await db
			.update(workItems)
			.set({
				pinnedOrder: sql`CASE 
					WHEN ${workItems.id} = ${current.id} THEN ${target.pinnedOrder}
					WHEN ${workItems.id} = ${target.id} THEN ${current.pinnedOrder}
					ELSE ${workItems.pinnedOrder}
				END`
			})
			.where(inArray(workItems.id, [current.id, target.id]));
	}
		...
		await db
			...
			.set({
				displayOrder: sql`CASE 
					WHEN ${workItems.id} = ${current.id} THEN ${target.displayOrder}
					WHEN ${workItems.id} = ${target.id} THEN ${current.displayOrder}
					ELSE ${workItems.displayOrder}
				END`
			})
			.where(inArray(workItems.id, [current.id, target.id]));
	}
	...
	async deleteGalleryImage(imageId: string): Promise<string | null> {
		const imgRes = await db.select().from(workImages).where(eq(workImages.id, imageId)).limit(1);
